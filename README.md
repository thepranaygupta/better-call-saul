...
- An **AI call-prep brief** with talking points, likely objections, and how to handle them
- A **one-tap outreach draft** for WhatsApp / email / SMS, in English / Hindi / Hinglish
...
| AI | Azure OpenAI | -- | Signal extraction, call briefs, message drafts via API routes |
...
- (Optional, once AI layer is added) Azure OpenAI credentials (`AZURE_OPENAI_API_KEY`, `AZURE_OPENAI_ENDPOINT`)
...
# Optional (once AI layer is added): AZURE_OPENAI_API_KEY, AZURE_OPENAI_ENDPOINT
...
pnpm db           # Start local MongoDB container
pnpm db:stop      # Stop local MongoDB container
pnpm seed         # Seed demo data (projects, users, leads, activities, signals)
pnpm setup        # db + seed in one step
```
